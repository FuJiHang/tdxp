...
Page({
  ...
  onLoad: function (options) {
    ...
    setTimeout(()=>{ 
      data.funtion.forEach(c=>{
        c.val=data.tuijian.data[options.index][c.data]
        c.choose=data.tuijian.data[options.index][c.chooseN]
      })
      that.setData({
        showTH:options.index,
        funtion:data.funtion,
      })
      this.toNext(options.index*data.wH)
    },500)
    ...
    if(options.isShare){
      ...
      app.getOpenId(function(a) {
        app.fg({
          action:'ShareSuccess',
          openId:a,
          Id:data.tuijian[options.index].Id,
          Type:1,
        }).then(r=>{
  ...
  getListTH(){
    ...
    app.fg({
      ...
      app.fh() 
      if(r.data.Status=='OK'){
        let datar=data.like?r.data.LikeData:r.data.Data,a=data.tuijian.data
  ...
  onShareAppMessage: function () {
    ...
    return {
      title: '头道惠',
      path: '/fuPackageA/fuJitterOne/fuJitterOne?chuang='+encodeURIComponent(JSON.stringify(data.tuijian.data))+"&page="+data.tuijian.page+
      '&UserId='+data.UserId+'&index='+data.showTH+'&isShare=true',
      imageUrl: "",
      success: (res) => {
        app.fa('转发成功')
      },
