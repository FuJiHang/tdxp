...
Page({
    ...
    // setTimeout(()=>{
      wx.chooseImage({ 
        ...
        success: res => {
          ...
          app.fl("正在上传中...")
          that.uploadImgs(logo,0,name).then(l=>{
            if(l){
              that.setData({
  ...
  onChange(event) {
    ...
    this.data.dataInfo.Orders.forEach(c=>{
      c.isChoose=event.detail
      if(c.isChoose){
        prjXfj.prj++
        if(c.IsIncludeRestorer==true) prjXfj.Xfj++
        if(c.IsIncludeCoupon==true) prjXfj.djj++
      }
    })
    this.setData({
  ...
  ChangeCheck(e){
    console.log(e)
    let aaa=e.currentTarget.dataset,data=this.data,that=this
    this.setData({
      ['dataInfo.Orders['+aaa.index+'].isChoose']:!aaa.data.isChoose
    })
    let prjXfj={
      prj:0,
      Xfj:0,
    ...
    data.dataInfo.Orders.forEach(c=>{
      if(c.isChoose){
        prjXfj.prj++
        if(c.IsIncludeRestorer==true) prjXfj.Xfj++
        if(c.IsIncludeCoupon==true) prjXfj.djj++
  ...
  getDataR(a){
    ...
    app.fg({
      ...
      PageSize:9999,
      //  ActivityId:2335,
      // openId:'oGsqu4qhkCcGOCQlAi4V0xH4A8WQ',
      // Role:0,
      // Type:0,
      // PageIndex:1,
      // PageSize:9999,
  ...
  submitFN(){
    if(!this.data.dataInfo.Orders) return app.fa('当前暂无业绩提交')
    if(this.data.choose[0]&&this.data.choose[1]&&this.data.choose[2]) {
      this.setData({
        show:true
      })
    }else app.fa('请确认项目数量和修复剂数量')
  },

  // 
  okFN(){
    let data=this.data,that=this,oId='',sss=0
    data.dataInfo.Orders.forEach(c=>{
      if(c.isChoose){
    ...
    app.fl()
    app.fg({
      action:'OnlineReportSend_new',
        // openId:'oGsqu4qBG6Icub2iLYzI3MJGH4iQ',
        openId:app.globalData.GetMembersInfo.openId,
        OrderIds:data.checked?'':oId,
  ...
  changeTea(e){
    let p={
      id:e.currentTarget.dataset.item.OrderId,
      sid:e.currentTarget.dataset.item.SkuId,
    }
    this.setData({
      project:p,
      ["teacher.show"]:true
  ...
  getTeach(){
    ...
    app.fg({ 
      ...
      openId:app.globalData.GetMembersInfo.openId,
      // openId:'oGsqu4g5MBYDuZXNS3Q6f05SIDpk',
      role: 'head',
